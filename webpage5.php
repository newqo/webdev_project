...
<body>
    <?php
        $stmt = $pdo->prepare("SELECT Users.national_id AS 'เลขบัตรประชาชน', Users.firstname AS 'ชื่อจริง' , Users.lastname AS 'นามสกุล', Scholarship.scholarship_name AS 'ชื่อทุน', Cost_of_living_status.amount AS 'ค่าครองชีพต่อเดือน', Reservation.reserve_date AS 'วันจอง', Reservation.reserve_time AS 'เวลาจอง', Reservation.queue_no AS 'คิวที่' 
                                FROM Reservation 
        ...
        while ($row = $stmt->fetch()) {
            ...
            echo "<td>" . $row ["ค่าครองชีพต่อเดือน"] . "</td>";
