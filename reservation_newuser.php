<?php 
    ...
    if (empty($_SESSION["national_id"]) ) { 
        ...
        $term = $pdo->prepare("SELECT Duration_id, Start_date , End_date FROM `Post_Duration` WHERE Reservation = 1 AND Event_status = 1 AND Duration_id LIKE '%NEW%' ORDER BY Start_date DESC");
        $term->execute();
